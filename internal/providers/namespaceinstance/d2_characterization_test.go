...
func TestD2BootstrapScriptExact(t *testing.T) {
	if got := fmt.Sprintf("%x", sha256.Sum256([]byte(namespaceToolBootstrapCommand()))); got != "3cc9c9e2840cf23ae57142f87b3abd36c37f11ede30d27ae8a14799b6c36a8e6" {
...
case "$last" in
 *crabbox-namespace-apt-update.log*) if [ "$D2_BOOTSTRAP_FAIL" = true ]; then exit 1; fi ;;
esac
exit 0
`
			...
			defer cancel()
			err := shared.PrepareSSHWithBootstrap(ctx, core.BaseConfig(), &target, b.rt.Stderr, "Namespace instance", namespaceToolBootstrapCommand())
			if fail {
				if err == nil || !strings.Contains(err.Error(), "Namespace instance tool bootstrap failed:") {
					t.Fatalf("err=%v", err)
				}
			} else if err != nil {
				t.Fatal(err)
			...
			text := string(data)
			bootstrap := strings.Index(text, namespaceToolBootstrapCommand())
			if bootstrap < 0 || !strings.Contains(text[:bootstrap], "true") {
				t.Fatalf("missing initial probe/bootstrap: %s", text)
			}
			tools := strings.Contains(text[bootstrap+len(namespaceToolBootstrapCommand()):], "command -v rsync")
			if tools == fail {
	...
	if err := shared.PrepareSSHWithBootstrap(ctx, core.BaseConfig(), &target, b.rt.Stderr, "Namespace instance", namespaceToolBootstrapCommand()); !errors.Is(err, context.Canceled) {
		t.Fatalf("err=%v", err)
	}
