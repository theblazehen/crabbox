...
type d1ReadClient struct {
	vercelSandboxClient
	get func(context.Context, string) (sandboxSummary, error)
}

func (c d1ReadClient) GetSandbox(ctx context.Context, id string) (sandboxSummary, error) {
	return c.get(ctx, id)
}
...
func TestD1ReadCharacterization(t *testing.T) {
	for _, mode := range []string{"ready", "ready before cancellation", "terminal", "wait terminal", "raw error", "canceled fetch", "deadline fetch", "parent deadline", "missing labels", "foreign labels", "ownership before cancellation", "foreign scope", "unclaimed"} {
		t.Run(mode, func(t *testing.T) {
			...
			sb := sandboxSummary{ID: "d1-sandbox", Metadata: b.ownershipMetadata(scope, leasePrefix+"d1-sandbox", "d1-status", core.Repo{})}
			...
			calls := 0
			api := d1ReadClient{vercelSandboxClient: fake, get: func(ctx context.Context, id string) (sandboxSummary, error) {
				calls++
				if id != sb.ID {
					t.Fatalf("id = %q", id)
				}
				...
				if mode == "raw error" || strings.Contains(mode, "fetch") || mode == "parent deadline" {
					return sandboxSummary{}, nativeErr
				}
				return sb, nil
			}}
			b.newClient = func(core.Config, core.Runtime) (vercelSandboxClient, error) { return api, nil }
			...
			calls = 0
			_, verifyErr := shared.VerifySandboxClaim(ctx, leaseID, sb.ID, b.validateClaimScope, api.GetSandbox, validateSandboxOwnership)
