...
func TestD2ObservationPrecedence(t *testing.T) {
	...
	for _, running := range []bool{true, false} {
		for _, stored := range []string{"", "ready", "error", "running", " "} {
			...
			view := b.serverFromContainer(inspectContainer{Status: inspectStatus{State: native}, Configuration: inspectConfiguration{ID: "crabbox-d2", Labels: claim.Labels}}, cfg)
			wantStatus := "stopped"
			if running {
				wantStatus = "running"
				if stored == "ready" {
