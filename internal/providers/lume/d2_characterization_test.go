...
func TestD2CleanupIdleBoundaries(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got, reason := shouldCleanup(server, claim, now)
...
func TestD2ObservationPrecedence(t *testing.T) {
	...
	for _, running := range []bool{true, false} {
		for _, stored := range []string{"", "ready", "error", "running", " "} {
			...
			view := b.serverFromInstance(lumeVM{Name: "crabbox-d2", Status: native, IPAddress: "192.0.2.1"}, claim, cfg)
			wantStatus := "stopped"
			if running {
				wantStatus = "running"
				if stored == "ready" {
			...
			wantLabel := stored
			if stored == "" || stored == "running" || (stored == "ready" && !running) {
			...
			if view.Labels["provider"] != providerName {
