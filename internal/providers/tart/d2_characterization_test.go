...
func TestD2ObservationPrecedence(t *testing.T) {
	...
	for _, running := range []bool{true, false} {
		for _, stored := range []string{"", "ready", "error", "running", " "} {
			...
			view := b.serverFromInstance(tartInstance{Name: "crabbox-d2", State: native}, claim, cfg)
			wantStatus := "stopped"
			if running {
				wantStatus = "running"
				if stored == "ready" {
			...
			if (view.Labels["last_touched_at"] != "") != true {
...
func TestD2ListDisplayFilterAndOrder(t *testing.T) {
	...
	payload := `[{"Name":"foreign","State":"running"},{"Name":"crabbox-second","State":"stopped"},{"Name":"crabbox-first","State":"running"}]`
	runner := &recordingRunner{responses: map[string]core.LocalCommandResult{commandKey([]string{"list", "--source", "local", "--format", "json"}): {Stdout: payload}}}
...
func TestD2RunningFlagDoesNotPromoteStoppedObservation(t *testing.T) {
	claim := core.LeaseClaim{Labels: map[string]string{"state": "ready"}}
	view := (&backend{}).serverFromInstance(tartInstance{Name: "crabbox-d2", State: "stopped", Running: true}, claim, core.BaseConfig())
	if view.Status != "stopped" || view.Labels["state"] != "ready" {
		t.Fatalf("inconsistent native observation lost precedence: %#v", view)
	}
}
