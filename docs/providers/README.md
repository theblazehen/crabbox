...
| [agent-sandbox-ssh](agent-sandbox.md#ssh-variant-agent-sandbox-ssh) | built-in; `ssh-lease` | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `cleanup`, `run-session` | `linux`; Kubernetes Agent Sandbox warm pool | `self-hosted`; GPU: unknown | Agent Sandbox SandboxClaim; owned SandboxClaim delete or retain | Core SSH sync/run against the existing pool | Same `agentSandbox` settings; root Linux amd64, existing executable login shell (no global shell-list requirement), initial upload tools, writable additive runtime/FHS paths, compatible existing tools, and pods/portforward RBAC; no package installation or account changes |
