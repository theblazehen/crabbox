...
| [agent-sandbox-ssh](agent-sandbox.md) | built-in; `ssh-lease` · delegated-sandbox | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `cleanup`, `run-session` | `linux`; Kubernetes Agent Sandbox warm pool with private SSH transport | `self-hosted`; GPU: unknown | Agent Sandbox SandboxClaim; owned SandboxClaim delete | Retained Kubernetes-hosted Linux workspaces with SSH and rsync | Requires kubectl port-forward RBAC, an SSH-compatible Linux amd64 warm pool, and explicit context |
