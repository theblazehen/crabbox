...
func TestInitializationRequestIdentityContract(t *testing.T) {
	...
	tests := []struct {
		...
		{"null key", `{"lease_id":"cbx_example","public_key":null}`, false},
		...
		{"obsolete host pin", `{` + base + `,"expected_host_key":` + string(encodedKey) + `}`, false},
		{"obsolete port pin", `{` + base + `,"expected_port":"23456"}`, false},
		{"key options", `{"lease_id":"cbx_example","public_key":"no-pty ` + key + `"}`, false},
		{"second key line", `{"lease_id":"cbx_example","public_key":"` + key + `\n` + key + `"}`, false},
		{"oversized input", `{"lease_id":"` + strings.Repeat("a", 16384) + `","public_key":` + string(encodedKey) + `}`, false},
	}
